...
first one, returning an empty module) — except `GI180` (info) and `GI190`
(warning), both pushed directly onto the diagnostics list without aborting parsing.
...
| `GI190` | warning | A `let` temp whose initializer reads variable V (directly, or transitively through an earlier temp that itself read V) is READ AGAIN after a later statement in the same body writes V. | In graph semantics a temp is a value edge with a per-activation cache keyed on its upstream inputs — a later read after V changes RE-EVALUATES against V's new state, not a value frozen at the `let`. TS reader intuition ("captured value") is wrong here; assign to a variable instead of a `let` to keep the old value. Straight-line/branch-forking analysis only (see `lintTempReuseAfterWrite`'s own doc comment for the exact, deliberately conservative scope: no cross-proc analysis, loop bodies walked once not as a back-edge, a write in EITHER side of an `if`/`switch`/async-or-stateful "outs" branch taints going forward). |
...
| `GI208` | error | A state slot is used by more than one `setDelay` call site. | `setDelay`'s graph node is keyed by state SLOT, not call site (the same slot's node is reused/memoized across references — see `getOrCreateStateNode`), but `cancelDelay`/`isActive` semantics attach to the NODE and assume a 1:1 slot↔call-site relationship. Two distinct `setDelay` statements sharing one slot would otherwise silently overwrite each other's `duration`/`done` wiring on the same node (nested — one inside the other's `done`-Cont — or sibling — two independent sites — same underlying clobber either way); this is now a hard export error instead. A `setDelay`'s OWN `done`-Cont legally re-entering ITSELF through its own slot (the self-rescheduling pattern `GI210`'s async exception covers) is NOT this — that's one call site, checked by object identity, not two. |
| `GI210` | error | A synchronous cycle was found while building a proc's body, or while assigning graph node indices (a value-edge ordering conflict). | Per `docs/design/ir-and-transpiler.md`'s "Why structuring always works": the *synchronous* proc call graph must be acyclic (cycles through `async.done` continuations are fine — that's a legitimate self-rescheduling delay, not this). A legal flow-only back-edge (e.g. that same self-rescheduling pattern) no longer trips this at the node-indexing stage either — only a cycle with no such async escape (a genuine value cycle, or a synchronous proc self/mutual call with no async boundary) still reports it. |
