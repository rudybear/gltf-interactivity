// parseModuleCs fidelity, two levels — mirrors packages/parse-py/test/
// parse.test.ts's own structure (read that file's header first; this only
// calls out where C#'s own comparison differs):
//
...
//     (from importGraph) is emitted through both @gltfi/emit-ts's
//     emitModule and @gltfi/emit-cs's emitModuleCs and reparsed with each
//     one's own parser (parseModule / parseModuleCs). Both resulting
//     modules are compared "modulo meta/names" (the same normalization
//     parse-py's own test file documents in full — duplicated here rather
//     than imported across package test boundaries, since vitest test
//     files aren't part of any package's public surface). Compared against
//     parseModule (TS) specifically, NOT parseModuleLua/parseModulePy: C#'s
//     own native-operator substitution is exactly as BROAD as TS's own (see
//     emit-cs's header note — both natively substitute int arithmetic AND
//     int/float/bool comparisons, unlike Python's/Lua's narrower float/bool-
//     only substitution), and both backends INLINE constant pointer-
//     template params into the path string (see emit-cs's `pointerCall` vs
//     emit-py's/emit-lua's own doc comments on NOT inlining) — so the C#
//     side needs the exact same `AMBIGUOUS_COMPARE_OPS`/
//     `inlinePointerConstants` canonicalization TS's own comparisons need,
//     making TS the natural cross-parser partner here (comparing against
//     Python/Lua would need to canonicalize in the OPPOSITE direction).
//
//  2. Whole-corpus "parses without ERROR diagnostics": every asset's
//     emitModuleCs output round-trips through parseModuleCs with zero
//     `severity: "error"` diagnostics (mirrors run-roundtrip-cs.ts's own
...
// process this whole file's `parseModuleCs` calls share gets torn down —
...
// Identical to parse-ts's/parse-py's own `inlinePointerConstants` — needed
// here too since BOTH emit-ts and emit-cs inline constant int-kind pointer
// template params into the literal path string (see this file's header
// note), so this is a no-op idempotent pass on either side, kept purely for
// defensive symmetry with the other cross-parser test files' shared shape.
...
// Mirrors parse-py's/parse-ts's/parse-lua's test files' identical helper:
// emit-ts AND emit-cs both rename `let` temps to short sequential `t<n>`
// ids (see either emitter's own `allocTemp`) — canonicalize both sides to
// first-encountered-order `t1`/`t2`/... (fresh per handler/proc body)
// before comparing, purely defensive since both sides already use this
// exact scheme.
...
// Identical normalization to parse-py's/parse-lua's/parse-ts's own test
// files (see any for the exhaustive per-IRStmt-kind rationale) —
// duplicated rather than imported across package test boundaries.
...
function stripForComparison(module: IRModule): unknown {
  return {
    ...
    // identifiers distinct from the ORIGINAL graph-node-id-derived name
    // emit-cs still round-trips unchanged, so comparing `.name` here would
    // spuriously fail even though both sides structurally agree.
...
describe("parseModuleCs - round-trips emitted C# to deep-equal IR (modulo meta)", () => {
  for (const [name, relPath] of CURATED) {
    it(`round-trips "${name}"`, () => {
      const original = loadModule(relPath);
      const { code } = emitModuleCs(original);
...
describe("parseModuleCs vs parseModule - cross-parser IR identity (modulo meta/names)", () => {
  for (const [name, relPath] of CURATED) {
    it(`agrees with parseModule on "${name}"`, () => {
      const original = loadModule(relPath);
      const csCode = emitModuleCs(original).code;
      const tsCode = emitModule(original).code;
      const { module: parsedCs, diagnostics: csDiags } = parseModuleCs(csCode);
      const { module: parsedTs, diagnostics: tsDiags } = parseModule(tsCode);
      expect(csDiags.filter((d) => d.severity === "error"), "parseModuleCs diagnostics").toEqual([]);
      expect(tsDiags.filter((d) => d.severity === "error"), "parseModule diagnostics").toEqual([]);
      expect(stripForComparison(parsedCs)).toEqual(stripForComparison(parsedTs));
    });
  }
});

describe("parseModuleCs - whole corpus parses without ERROR diagnostics", () => {
  ...
  for (const asset of assets) {
    it(`parses "${asset.name}" cleanly`, () => {
      ...
      if (importErrors.length > 0) {
        // Mirrors run-roundtrip-cs.ts's own SKIP convention: a graph
      ...
      const { diagnostics } = parseModuleCs(code);
      const errors = diagnostics.filter((d) => d.severity === "error");
      expect(errors, JSON.stringify(errors, null, 2)).toEqual([]);
    });
  }
