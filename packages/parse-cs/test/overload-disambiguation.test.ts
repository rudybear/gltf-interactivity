...
// test/overload-disambiguation.test.ts and packages/parse-lua/parse-py's
// sibling tests of the same name): disambiguateOverload (src/index.ts)
...
//
// As in parse-lua's/parse-py's sibling tests, these build a tiny IRModule
// with the M.Transform op ALREADY correctly resolved, emit it to C#
// (emitModuleCs always spells a vector/matrix const as a plain `new
// double[] { ... }` array-creation literal — reproducing the exact
// all-literal-args call shape the bug needs), then re-parse that C# and
// assert parseModuleCs's own disambiguateOverload recovers the same
// overload. Same emit -> parse round-trip idiom this package's own
// handlers.test.ts uses; `afterAll(closeParser)` mirrors that file's own
// shared-Roslyn-session cleanup.
...
describe("parseModuleCs - disambiguateOverload literal-shape filtering (#18/#21)", () => {
