...
// test/overload-disambiguation.test.ts and its parse-lua/parse-py/parse-cs
// siblings of the same name): disambiguateOverload (src/index.ts) skipped
// literal-ish args entirely, so an all-literal call to a fn name with
// multiple FIXED-type candidate rows fell through to candidates[0]
...
// literals' actual lengths.
//
// As in the sibling tests, these build a tiny IRModule with the m.transform
// op ALREADY correctly resolved, emit it to GDScript (emitModuleGd always
// spells a vector/matrix const as a plain `[...]` array literal —
// reproducing the exact all-literal-args call shape the bug needs), then
// re-parse that GDScript and assert parseModuleGd's own disambiguateOverload
// recovers the same overload. Same emit -> parse round-trip idiom this
// package's own handlers.test.ts uses; unlike parse-py/parse-cs there is no
// `closeParser` to call — parseModuleGd is a synchronous, hand-rolled
// parser with no subprocess session (see index.ts's own header note).
...
describe("parseModuleGd - disambiguateOverload literal-shape filtering (#18/#21)", () => {
