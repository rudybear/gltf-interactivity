// Round-trip conformance runner (Lua surface): GLB -> importGraph ->
// emitModuleLua -> parseModuleLua -> exportGraph -> run graph' on the
// INTERPRETER via the judge protocol. Mirrors run-roundtrip.ts EXACTLY (see
// that file's own header comment for the full rationale) except the
// TS-surface emit/parse pair (@gltfi/emit-ts's emitModule / @gltfi/parse-ts's
// parseModule) is swapped for the Lua-surface pair (@gltfi/emit-lua's
// emitModuleLua / @gltfi/parse-lua's parseModuleLua) — everything downstream
// of that swap (exportGraph, validateGraph, the interpreter judge, the
// informational equivalentGraphs check) is the same shared, backend-neutral
// machinery run-roundtrip.ts already exercises for the TS surface. A passing
// run here is evidence that @gltfi/parse-lua reconstructs semantically
// equivalent IR from emit-lua's output, independent of the SEPARATE
// compiled-Lua-VM conformance path (run-compiled-lua.ts), which never
// touches @gltfi/parse-lua at all.
...
async function runOne(asset: { name: string; glbPath: string; testPath: string }): Promise<RunOutcome> {
  try {
    ...
    const { module: module2, diagnostics: parseDiags } = parseModuleLua(code);
