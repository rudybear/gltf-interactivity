// Round-trip conformance runner (Python surface): GLB -> importGraph ->
// emitModulePy -> parseModulePy -> exportGraph -> run graph' on the
// INTERPRETER via the judge protocol. Mirrors run-roundtrip-lua.ts EXACTLY
// (see that file's own header comment for the full rationale) except the
// Lua-surface emit/parse pair (@gltfi/emit-lua's emitModuleLua /
// @gltfi/parse-lua's parseModuleLua) is swapped for the Python-surface pair
// (@gltfi/emit-py's emitModulePy / @gltfi/parse-py's parseModulePy) —
...
// shared, backend-neutral machinery run-roundtrip.ts/run-roundtrip-lua.ts
// already exercise. A passing run here is evidence that @gltfi/parse-py
// reconstructs semantically equivalent IR from emit-py's output, independent
// of the SEPARATE compiled-Python conformance path (run-compiled-py.ts),
// which never touches @gltfi/parse-py at all.
//
// @gltfi/parse-py's `parseModulePy` lazily spawns ONE persistent `python3
// harness.py` process the first time it's called (see packages/parse-py/src/
// session.ts) and reuses it for every asset in this run; `closeParser()` is
// called once at the very end so that shared process (and its FIFO fds) is
// torn down and this script's own process exits promptly instead of hanging
// on a lingering child.
...
import { closeParser, parseModulePy } from "@gltfi/parse-py";
...
async function runOne(asset: { name: string; glbPath: string; testPath: string }): Promise<RunOutcome> {
  try {
    ...
    const { module: module2, diagnostics: parseDiags } = parseModulePy(code);
