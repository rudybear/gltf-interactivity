...
// emitted code only — the IR's own name is left untouched (still used for
// nothing but @gltfi/ir/import.ts's own within-module de-duplication).
// ---------------------------------------------------------------------------
...
function computeStateSlotDisplayNames(slots: IRStateSlot[]): string[] {
...
// independent of whether emit-ts renders that add natively or via `m.*`.
// Only considered for `int`-typed variables (a "counter" reads oddly for a
// float/bool/vector).
...
function computeVariableDisplayNames(module: IRModule): string[] {
